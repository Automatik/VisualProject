...
for ii = 1:testSize
    features = testHogFeatures(ii,:);
    label = cellstr(pred(ii));
    
    % calculate euclidean similarity metric
%     D = pdist2(trainHogFeatures,features,'euclidean'); 
    % normalize with max distance
%     maxDistance = max(D);
%     D = D ./ maxDistance;
    % calculate cosine similarity metric
    D = pdist2(trainHogFeatures,features,'cosine');

    % correlation metric
%     D = pdist2(trainHogFeatures, features, 'correlation');
